...
func TestConfigListPrintsFullConfig(t *testing.T) {
	...
	cmd := NewConfigCommand(func() (*app.ConfigApp, error) {
		...
		if err := configApp.Set(context.Background(), "output", "json"); err != nil {
			t.Fatalf("Set() error = %v", err)
		}
		if err := configApp.Set(context.Background(), "default_project", "Work"); err != nil {
			t.Fatalf("Set() error = %v", err)
		}
	...
	if !strings.Contains(stdout.String(), "output: json") {
		t.Fatalf("stdout = %q, want output", stdout.String())
	}
	if stderr.Len() != 0 {
